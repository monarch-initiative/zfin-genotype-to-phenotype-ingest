...
```bash
cd zfin-genotype-to-phenotype-ingest
make install
make test
```
...
Download the data for the zfin_genotype_to_phenotype_ingest transform:

```bash
poetry run zfin_genotype_to_phenotype_ingest download
```

To run the Koza transform for zfin-genotype-to-phenotype-ingest:

```bash
poetry run zfin_genotype_to_phenotype_ingest transform
```

To see available options:

```bash
poetry run zfin_genotype_to_phenotype_ingest download --help
# or
poetry run zfin_genotype_to_phenotype_ingest transform --help
```
