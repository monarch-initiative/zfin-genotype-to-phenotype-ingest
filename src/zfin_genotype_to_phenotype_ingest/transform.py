...
from biolink_model.datamodel.pydanticmodel_v2 import GenotypeToPhenotypicFeatureAssociation, KnowledgeLevelEnum, AgentTypeEnum # Replace * with any necessary data classes from the Biolink Model
...
while (row := koza_app.get_row()) is not None:
    ...
    zp_key_elements = [row["Affected Structure or Process 1 subterm ID"],
    ...
    if key in seen_records:
        logger.debug("Duplicate record found presumably for differences in life stages, Record={}, LifeStage={}".format(key, row["End Stage Name"]))
        continue
    else:
